...
# clean text and convert it to training format
relevant_users = set(['Wanderer', 'Pandoge', 'blownking', 'crmbob5', 'Fl3tchr', 'mrbabybob', 'BenjiMcmuscles', 'ztoms', 'mynameismili', 'Dingo'])
...
for msg in messages:
    ...
    if content == prev_content and author == prev_author:
        continue
    ...
    if author == prev_author:
        ...
        prev_author = author
        # remove users the programmer doesn't care about
        if author not in relevant_users:
            author = 'rando'
        text = text + f"<|end|> \n {author}: \n {content} \n "
